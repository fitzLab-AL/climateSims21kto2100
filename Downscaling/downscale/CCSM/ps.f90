!downscale surface pressure
!gfortran -O3 -o ps ps.f90 read_CCSM.o -I/Users/djlorenz/Research/netcdf/include -I/Users/djlorenz/lib -L/Users/djlorenz/Research/netcdf/lib -L/Users/djlorenz/lib -lnetcdff -lnetcdf  -llapack -lblas -lmylib
program ps
  ...
  call get_CCSM(minlon,maxlon,minlat,maxlat,-7,3,'PS')
  call fill_miss(nlonc,nlatc,4*ndecade_ref,xc)
  xc(:,:,1,:,:) = merge(xc(:,:,1,:,:),missing, &
       xc(:,:,1,:,:)/=missing)
  ...
  call get_CCSM(minlon,maxlon,minlat,maxlat,decade1,decade2,'PS')

  call ctlwrite('ps.ctl','ps',lons=lonsc,lats=latsc, &
       nlev=ndecade,minlev=0.01*decade1,dlev=0.01,nens=4)
  open(1,file='ps.dat',access='direct',form='unformatted',recl=4*nlonc*nlatc)

  do idecade=1,ndecade
!     print*, idecade
     call fill_miss(nlonc,nlatc,4,xc(:,:,:,:,idecade))
     y = merge(xc(:,:,1,:,idecade),missing, &
          xc(:,:,1,:,idecade)/=missing)
     do isea=1,4
        y(:,:,isea) = merge(y(:,:,isea)/xave(:,:,isea),missing, &
