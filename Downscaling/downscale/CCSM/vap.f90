!downscale vapor pressure
!gfortran -O3 -o vap vap.f90 read_CCSM.o -I/Users/djlorenz/Research/netcdf/include -I/Users/djlorenz/lib -L/Users/djlorenz/Research/netcdf/lib -L/Users/djlorenz/lib -lnetcdff -lnetcdf  -llapack -lblas -lmylib
program vap
  ...
  call get_CCSM(minlon,maxlon,minlat,maxlat,-7,3,'Q2M PS')
  call fill_miss(nlonc,nlatc,2*4*ndecade_ref,xc)
  xc(:,:,1,:,:) = merge(xc(:,:,1,:,:)*xc(:,:,2,:,:),missing, &
       xc(:,:,1,:,:)/=missing)
  ...
  call get_CCSM(minlon,maxlon,minlat,maxlat,decade1,decade2,'Q2M PS')

  call ctlwrite('vap.ctl','vap',lons=lonsc,lats=latsc, &
       nlev=ndecade,minlev=0.01*decade1,dlev=0.01,nens=4)
  open(1,file='vap.dat',access='direct',form='unformatted',recl=4*nlonc*nlatc)

  do idecade=1,ndecade
!     print*, idecade
     call fill_miss(nlonc,nlatc,2*4,xc(:,:,:,:,idecade))
     y = merge(xc(:,:,1,:,idecade)*xc(:,:,2,:,idecade),missing, &
