!gfortran -O3 -I/Users/djlorenz/Research/netcdf/include -I/Users/djlorenz/lib -c read_ECBilt.f90
module read_ECBilt
  ...
  real, allocatable, public :: xc(:,:,:,:,:) ! lon,lat,var,month,decade

contains

  ! time from = -2200 to 3 (unit == decades)

  subroutine get_ECBilt(lon11,lon22,lat1,lat2,decade1,decade2,vars)
    ...
    integer, intent(in) :: decade1,decade2
    ...
    if(nvar == 0) then
       print*, 'ERROR: get_ECBilt: the string "vars" is empty'
       stop
    endif
    
    call handle_err(nf90_open('/Users/djlorenz/Research/jack/ECBilt-CLIO/special/'// &
    ...
    if(lon1==missing .or. lon2==missing) then
       ...
       if(dmin==0 .or. dmax==0) then
          print*, 'ERROR: get_ECBilt: longitude range outside dataset'
          stop
       endif
    endif
    if(dmax>=dmin) then
    ...
    if(lat1==missing .or. lat2==missing) then
       ...
       enddo
       if(dmin==0 .or. dmax==0) then
          print*, 'ERROR: get_ECBilt: latitude range outside dataset'
          stop
       endif
    endif
    if(dmax<dmin) then
    ...
    ! decade
    if(decade1<-2100 .or. decade1>-1) then
       print*, 'ERROR: get_ECBilt: decade out of range'
       print*, 'decade = ',decade1,decade2
       stop
    endif
    if(decade2<-2100 .or. decade2>-1) then
       print*, 'ERROR: get_ECBilt: decade out of range'
       print*, 'decade = ',decade1,decade2
       stop
    endif
    if(decade1 > decade2) then
       print*, 'ERROR: get_ECBilt: decade1 > decade2'
       stop
    endif
    time = decade1 + 2101
    ntc = decade2 - decade1 + 1
