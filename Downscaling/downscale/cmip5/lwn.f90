!gfortran -O3 -o lwn lwn.f90 -I/Users/djlorenz/Research/netcdf/include -I/Users/djlorenz/lib -L/Users/djlorenz/Research/netcdf/lib -L/Users/djlorenz/lib -lnetcdff -lnetcdf  -llapack -lblas -lmylib
program temp0
  ...
  do imodel = 1,nmodel
     ...
     ! check sign and convert to net longwave:
     temp = sum(x1(:,:,1,:,1),x1(:,:,1,:,1)/=missing)
     if(temp < 0.) then
        print*, 'FLIP SIGN 1'
        stop
     endif
     temp = sum(x1(:,:,2,:,1),x1(:,:,2,:,1)/=missing)
     if(temp < 0.) then
        print*, 'FLIP SIGN 2'
        stop
     endif
     x1(:,:,1,:,:) = merge(x1(:,:,1,:,:) - x1(:,:,2,:,:), missing, &
     ...
     do iyear = year1,year2
        ...
        do imonth = 1,12
           x1(:,:,1,imonth,iyear-year1+1) = x1(:,:,1,imonth,iyear-year1+1) - &
                xave(:,:,imonth)
        enddo
     ...
     do iscen = 1,2
        ...
        temp = sum(x1(:,:,2,:,1),x1(:,:,2,:,1)/=missing)
        if(temp < 0.) then
           print*, 'FLIP SIGN 2',trim(scens(iscen))
           stop
        endif
        x1(:,:,1,:,:) = merge(x1(:,:,1,:,:) - x1(:,:,2,:,:), missing, &
             x1(:,:,1,:,:) /= missing .and. x1(:,:,2,:,:) /= missing)
        ...
        do iyear = year11,year22
           ...
           do imonth = 1,12
              x1(:,:,1,imonth,iyear-year11+1) = x1(:,:,1,imonth,iyear-year11+1) - &
                   xave(:,:,imonth)
           enddo
