!gfortran -O3 -o swu swu.f90 -I/Users/djlorenz/Research/netcdf/include -I/Users/djlorenz/lib -L/Users/djlorenz/Research/netcdf/lib -L/Users/djlorenz/lib -lnetcdff -lnetcdf  -llapack -lblas -lmylib
program temp0
  ...
  real :: temp
  integer*2, allocatable :: ixf(:,:),ixf_swd(:,:)

  real, parameter :: sf=0.1, ao=0.0
  
  character (len=400) :: fname
  integer :: imodel,iscen,iyear,imonth,ivar,ilon,ilat,it,ncid,vid
  ...
  xfave = x1(:,:,1,:,1) - x1(:,:,2,:,1)
  deallocate(x1)
  allocate(xf(nlonf,nlatf,12),ixf(nlonf,nlatf),ixf_swd(nlonf,nlatf))
  ...
  do imodel = 1,nmodel
     ...
     call handle_err(nf90_open('/Users/djlorenz/Research/jack/final/cmip5/historical/'// &
          trim(models(imodel))//'/swd.nc',nf90_nowrite,ncid))
     call handle_err(nf90_inq_varid(ncid,'swd',vid))
     ...
     do iyear = year1,year2
        ...
        do imonth = 1,12
           it = it + 1
           ! read swd to enforce range:
           call handle_err(nf90_get_var(ncid,vid,ixf_swd,start=[1,1,it],count=[nlonf,nlatf,1]))
           do ilat = 1,nlatf
              do ilon = 1,nlonf
                 if( xf(ilon,ilat,imonth) /= missing ) then
                    ...
                    if(ixf(ilon,ilat) > ixf_swd(ilon,ilat)) then
                       ixf(ilon,ilat) = ixf_swd(ilon,ilat)
                    endif
                 else
     ...
     do iscen = 1,2
        ...
        call handle_err(nf90_open('/Users/djlorenz/Research/jack/final/cmip5/'//trim(scens(iscen))//'/'// &
             trim(models(imodel))//'/swd.nc',nf90_nowrite,ncid))
        call handle_err(nf90_inq_varid(ncid,'swd',vid))
