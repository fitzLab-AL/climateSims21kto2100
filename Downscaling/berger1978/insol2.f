c  phi == latitude (degrees)
c  tin == year relative to 1950. in thousands
c  ma == month
c  ja == day
c  ecc == eccentricity
c  pre == prec. param.
c  perh == long. perh.
c  xob == obliquity
c  ww == daily insolation
c  dayl == day length
c                        <- input <-|-> output ->
      subroutine insol(phi,tin,ma,ja,ecc,pre,perh,xob,ww,dayl)
     ...
     *(78),COP(78)                                                      PAL00030
      ...
      save first
      data first /.true./
      save ae,be,ce,aob,bob,cob,aop,bop,cop
      parameter(IOPT=2,NEFF=19,NOBB=47,NOPP=78)
C                                                                       PAL00040
C                                                                       PAL00050
...
c                                                                       PAL00411
ccc      WRITE(6,5996)                                                     PAL00412
ccc 5996 FORMAT(1X,'THIS PROGRAMME COMPUTES THE TOTAL DAILY IRRADIATION RECPAL00413
ccc     .EIVED AT THE TOP '/'OF THE ATMOSPHERE FOR A GIVEN LATITUDE AND TIMPAL00414
ccc     .E IN THE YEAR (IN KJ M-2)')                                       PAL00415
      if(first) then
...
C                                                                       PAL01020
ccc      WRITE(6,5999)                                                     PAL01721
ccc 5999 FORMAT(1X,'ENTER SUCCESSIVELY : '/ 'PHI : LATITUDE'/'T   : TIME (IPAL01722
ccc     .N 1000 YEAR'/'      0 FOR THE PRESENT'/'       NEGATIVE IN THE PASPAL01723
ccc     .T'/'       POSITIVE IN THE FUTURE'/                               PAL01724
ccc     .'IOPT: = 1 TRUE LONG. SUN TLS'/'      = 2  MONTH MA - DAY J'/     PAL01725
ccc     .'NEFF  NOBB  NOPP :  NUMBER  OF TERMS KEPT FOR COMPUTATION        PAL01726
ccc     .OF'/'THE EARTH ORBITAL ELEMENTS.'/'THEY ARE 19 47 78 AT THE MAXIMUPAL01727
ccc     .M'/'THEY CAN BE REDUCED TO 19 18 9 RESPECTIVELY FOR A MINIMUM ACCUPAL01728
ccc     .RACY')                                                            PAL01729
C                                                                       PAL01030
ccc      READ(5,*) PHI,T,IOPT,NEFF,NOBB,NOPP                               PAL01035
C                                                                       PAL01040
C   3.NUMERICAL VALUE FOR ECC PRE XOB                                   PAL01050
C************************************                                   PAL01060
C       T IS NEGATIVE FOR THE PAST   T IS IN 1000 YEARS                 PAL01070
C                                                                       PAL01080
      T=tin*1000.0D0                                                      PAL01090
      XES=0.0D0                                                         PAL01100
      XEC=0.0D0                                                         PAL01110
...
ccc      WRITE(6,6003) PHI                                                 PAL01740
ccc 6003 FORMAT(1X,'LATITUDE =',F6.1)                                      PAL01750
ccc      WRITE(6,6000) T,ECC,PRE,PERH,XOB                                  PAL01760
ccc 6000 FORMAT(1X,'TIME =',F10.1,3X,'ECCENTRICITY =',F8.5,/,20X,'PREC. PARPAL01770
ccc     *AM. =',F8.5,/,20X,'LONG. PERH. =',F7.1,/,20X,'OBLIQUITY =',F7.3)  PAL01780
C                                                                       PAL01790
...
C                                                                       PAL01880
ccc      WRITE(6,5998)                                                     PAL01882
ccc 5998 FORMAT(/1X,'ENTER TRUE LONGITUDE OF SUN (TLS)'/'TLS=0,30,...300,33PAL01884
ccc     .0 RESPECTIVELY FOR MARCH ... JANUARY, FEBRUARY')                  PAL01886
C                                                                       PAL01888
ccc      READ(5,*) TLS                                                     PAL01890
C                                                                       PAL01900
      CALL DAYINS(ECC,XL,SO,TLS,PHI,PIR,PI,TEST,SF,WW,DAYL)             PAL01910
ccc      WRITE(6,6001) TLS                                                 PAL01920
ccc 6001 FORMAT(1X,'TRUE LONG. SUN =',F7.1)                                PAL01930
ccc      WRITE(6,6002) WW,DAYL                                             PAL01940
ccc 6002 FORMAT(1X,'DAY INSOL. =',F7.0,1X,'KJ M(-2) DAY(-1) ',4X,'LENGTH DAPAL01950
ccc     * =',F6.2,1X,'HOURS')                                              PAL01960
      GO TO 101                                                         PAL01970
C                                                                       PAL01980
...
C                                                                       PAL02052
ccc      WRITE(6,5997)                                                     PAL02054
ccc 5997 FORMAT(/1X,'ENTER CALENDAR DATE  (MONTH MA - DAY JA) ')           PAL02056
C                                                                       PAL02060
ccc      READ(5,*) MA,JA                                                   PAL02070
C                                                                       PAL02080
...
ccc      WRITE(6,6010) MA,JA,TLS                                           PAL02260
ccc 6010 FORMAT(1X,'MONTH =',I3,3X,'DAY =',I3,3X,'TLS =',F7.1)             PAL02270
ccc      WRITE(6,6002) WW,DAYL                                             PAL02280
C                                                                       PAL02290
ccc  101 WRITE(6,6100)                                                     PAL02300
ccc 6100 FORMAT(/70('*')/)                                                 PAL02310
C                                                                       PAL02320
ccc      GO TO 100                                                         PAL02330
ccc      STOP                                                              PAL02340
 101  continue
      return
      END                                                               PAL02350
